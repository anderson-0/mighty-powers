...
<workflow>
      ...
      <check if="no ready-for-dev or in-progress story found">
        <output>No ready-for-dev stories found in sprint-status.yaml
          ...
          **What would you like to do?**
          1. Run `mighty-powers:create-story` to create next story from epics with comprehensive context
        ...
        <check if="user chooses '1'">
          <action>HALT - Run mighty-powers:create-story to create next story</action>
        </check>
  ...
  <step n="10" goal="Completion communication and user support">
    ...
    <action>Recommended next steps (flexible based on project setup):
      ...
      - Run `mighty-powers:code-review` workflow for peer review
      - Optional: If Test Architect module installed, run `/bmad:tea:automate` to expand guardrail tests
    </action>

    <output>**Tip:** For best results, run `mighty-powers:code-review` using a **different** LLM than the one that implemented this story.</output>
    <check if="{sprint_status} file exists">
