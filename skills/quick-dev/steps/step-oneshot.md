...
Invoke the `mighty-powers:adversarial-review` skill in a subagent with the changed files. The subagent gets NO conversation context -- to avoid anchoring bias. If no sub-agents are available, write the changed files to a review prompt file in `{implementation_artifacts}` and HALT. Ask the human to run the review in a separate session and paste back the findings.
...
3. **Suggested Review Order** -- append after Intent. Build using the same convention as `./steps/step-05-present.md` "Generate Suggested Review Order" (spec-file-relative links, concern-based ordering, ultra-concise framing).
