...
- **Blind hunter** -- receives `{diff_output}` only. No spec, no context docs, no project access. Invoke via the `mighty-powers:adversarial-review` skill.
- **Edge case hunter** -- receives `{diff_output}` and read access to the project. Invoke via the `mighty-powers:adversarial-review` skill with edge-case focus.
