...
1. Bind `{doc_workspace}` and create the memlog with `python3 ${CLAUDE_PLUGIN_ROOT}/tools/lib/memlog.py init --workspace {doc_workspace} --field topic="<topic>" [--field goal="<goal>"]`. It remains the canonical source every artifact derives from.
