...
- `{workflow.<name>}` resolves to fields in `customize.toml`'s `[workflow]` table (overrides win per Mighty Powers merge rules).
...
1. Resolve customization: `python3 ${CLAUDE_PLUGIN_ROOT}/tools/lib/resolve-customization.py --skill {skill-root} --key workflow`. On failure, read `{skill-root}/customize.toml` directly and use defaults.
...
3. Load `{project-root}/.mighty-powers/config.yaml` (+ `config.user.yaml` if present). Resolve `{user_name}`, `{communication_language}`, `{document_output_language}`, `{planning_artifacts}`, `{project_name}`, `{date}`. Missing keys → neutral defaults; never block.
4. If headless, follow `references/headless.md` for the whole run. Otherwise greet the user **by name** using `{user_name}` and **in their language** using `{communication_language}` — and stay in `{communication_language}` for every turn for the entire run, not just the greeting. In the greeting, let the user know that at any point they can invoke `mp:party-mode` for multi-agent perspectives or `mp:advanced-elicitation` for deeper exploration on a specific section. Then scan for misroute on the first message: if the signal points elsewhere (game → a game-design workflow (not bundled in Mighty Powers); express build → `mp:quick-dev`; one-pager → `mp:product-brief`; vet product idea → `mp:prfaq`; agent skill or custom agent → `mp:writing-skills`), suggest they might want the other options before continuing.
...
7. **Close.** Set `prd.md` frontmatter `status: final` and `updated` to `{date}` so future invocations distinguish this PRD from in-progress drafts. Record finalization to `.decision-log.md`. Share artifact paths. Common next: `mp:create-ux-design`, `mp:architecture`, `mp:create-epics`; invoke `mp:help` for authoritative routing.
