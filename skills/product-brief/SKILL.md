...
At the opening greeting, let the user know they can invoke `mp:party-mode` for multi-agent perspectives or `mp:advanced-elicitation` for deeper exploration at any point.
...
5. Load `{project-root}/.mighty-powers/config.yaml` (and `config.user.yaml` if present). Resolve `{user_name}`, `{communication_language}`, `{document_output_language}`, `{planning_artifacts}`, `{project_name}`, `{date}`.
...
4. Tell the user it is ready: local paths and external destinations (URLs returned from handoffs). Invoke `mp:help` to suggest what next steps make sense in the bmad method ecosystem.
