...
Load config from `.mighty-powers/config.yaml` and resolve:
...
<step n="1" goal="Determine target story">
  ...
  <check if="sprint status file does NOT exist">
    ...
    <output>
      **Required Options:**
      1. Run `mighty-powers:sprint` to initialize sprint tracking (recommended)
    ...
    <check if="user chooses '1'">
      <output>Run mighty-powers:sprint workflow first to create sprint-status.yaml</output>
      <action>HALT - User needs to run mighty-powers:sprint</action>
    </check>
  ...
  <check if="no user input provided">
    ...
    <check if="no backlog story found">
      <output>No backlog stories found in sprint-status.yaml
        ...
        **Options:**
        1. Run mighty-powers:sprint to refresh story tracking
        2. Load PM agent and run mighty-powers:correct-course to add more stories
        3. Check if current sprint is complete and run mighty-powers:retrospective
      </output>
      <action>HALT</action>
    </check>
    ...
    <check if="this is first story in epic {{epic_num}}">
      ...
      <check if="epic status is not one of: backlog, contexted, in-progress, done">
        ...
        <output>Please fix sprint-status.yaml manually or run mighty-powers:sprint to regenerate</output>
        <action>HALT - Cannot proceed</action>
      </check>
...
<step n="3" goal="Architecture analysis for developer guardrails">
  <critical>ARCHITECTURE INTELLIGENCE - Extract everything the developer MUST follow!</critical> **ARCHITECTURE DOCUMENT ANALYSIS:** <action>Systematically
...
<step n="6" goal="Update sprint status and finalize">
  ...
  <output>**ULTIMATE STORY CONTEXT CREATED, {user_name}!**
    ...
    **Next Steps:**
    ...
    2. Run dev agents `mighty-powers:dev-story` for optimized implementation
    3. Run `mighty-powers:code-review` when complete (auto-marks done)
    4. Optional: If Test Architect module installed, run `/bmad:tea:automate` after `mighty-powers:dev-story` to generate guardrail tests
